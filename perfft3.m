% PERFFT3  3D Fourier transform of Moisan's periodic image component
%
% Usage: [P, S, p, s] = perfft3(im)
%
% Argument:  im - Image to be transformed
% Returns:    P - 3D fft of periodic image component
%             S - 3D fft of smooth component
...
%Joshua Morgan 2017
%Adapted to 3D filter
%All credit to original author

function [P, S, p, s] = perfft3(im)
    ...
    [cx, cy, cz] = meshgrid(2*pi*[0:cols-1]/cols, 2*pi*[0:rows-1]/rows, 2*pi*[0:slices-1]/slices);    
    
    % Generate FFT of smooth component
    S = fftn(s)./(2*(3 - cos(cx) - cos(cy) - cos(cz))); %MAYBE CHANGE 2 TO 3?
    
    % The (1,1) element of the filter will be 0 so S(1,1) may be Inf or NaN
    S(1,1,1) = 0;          % Enforce 0 mean 

    P = fftn(im) - S;    % FFT of periodic component
